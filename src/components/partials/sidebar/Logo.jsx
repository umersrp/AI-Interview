// import React from "react";
// import { Link } from "react-router-dom";
// import Icon from "@/components/ui/Icon";
// import useDarkMode from "@/hooks/useDarkMode";
// import useSidebar from "@/hooks/useSidebar";
// import useSemiDark from "@/hooks/useSemiDark";
// import useSkin from "@/hooks/useSkin";

// // import images
// import MobileLogo from "@/assets/images/logo/new-new-logo.png";
// import MobileLogoWhite from "@/assets/images/logo/new-new-logo.png";

// const SidebarLogo = ({ menuHover }) => {
//   const [isDark] = useDarkMode();
//   const [collapsed, setMenuCollapsed] = useSidebar();
//   // semi dark
//   const [isSemiDark] = useSemiDark();
//   // skin
//   const [skin] = useSkin();
//   return (
//     <div
//       className={` logo-segment flex justify-between items-center bg-white dark:bg-slate-800 z-[9] py-6  px-4 
//       ${menuHover ? "logo-hovered" : ""}
//       ${skin === "bordered"
//           ? " border-b border-r-0 border-slate-200 dark:border-slate-700"
//           : " border-none"
//         }
      
//       `}
//     >
//       <Link to="/dashboard">
//         <div className="flex justify-center items-center w-full">
//           {!isDark && !isSemiDark ? (
//             <img src={MobileLogo} alt="logo" className="w-40 h-20 object-contain" />
//           ) : (
//             <img src={MobileLogoWhite} alt="logo" className="w-40 h-20 object-contain" />
//           )}
//         </div>
//       </Link>

//       {/* {(!collapsed || menuHover) && (
//         <div
//           onClick={() => setMenuCollapsed(!collapsed)}
//           className={`h-4 w-4 border-[1.5px] border-slate-900 dark:border-slate-700 rounded-full transition-all duration-150
//           ${
//             collapsed
//               ? ""
//               : "ring-2 ring-inset ring-offset-4 ring-black-900 dark:ring-slate-400 bg-slate-900 dark:bg-slate-400 dark:ring-offset-slate-700"
//           }
//           `}
//         ></div>
//       )} */}
//     </div>
//   );
// };

// export default SidebarLogo;

import React from "react";
import { Link } from "react-router-dom";
import Icon from "@/components/ui/Icon";
import useDarkMode from "@/hooks/useDarkMode";
import useSidebar from "@/hooks/useSidebar";
import useSemiDark from "@/hooks/useSemiDark";
import useSkin from "@/hooks/useSkin";

// FIX: Use the correct logo files that actually exist
import MobileLogo from "@/assets/images/logo/new-logo.png";  // Changed from new-new-logo.png
import MobileLogoWhite from "@/assets/images/logo/new-logo.png"; // Changed from new-new-logo.png

const SidebarLogo = ({ menuHover }) => {
  const [isDark] = useDarkMode();
  const [collapsed, setMenuCollapsed] = useSidebar();
  // semi dark
  const [isSemiDark] = useSemiDark();
  // skin
  const [skin] = useSkin();
  
  return (
    <div
      className={`logo-segment flex justify-between items-center bg-white dark:bg-slate-800 z-[9] py-6 px-4 
      ${menuHover ? "logo-hovered" : ""}
      ${
        skin === "bordered"
          ? " border-b border-r-0 border-slate-200 dark:border-slate-700"
          : " border-none"
      }
      `}
    >
      <Link to="/dashboard">
        <div className="flex justify-center items-center w-full">
          {/* FIX: Use the same logic as your Header component */}
          {!isDark && !isSemiDark ? (
            <img 
              src={MobileLogo} 
              alt="logo" 
              className="w-32 h-16 object-contain" // Slightly smaller for sidebar
            />
          ) : (
            <img 
              src={MobileLogoWhite} 
              alt="logo" 
              className="w-32 h-16 object-contain"
            />
          )}
        </div>
      </Link>

      {/* Optional collapse button */}
      {(!collapsed || menuHover) && (
        <div
          onClick={() => setMenuCollapsed(!collapsed)}
          className={`h-4 w-4 border-[1.5px] border-slate-900 dark:border-slate-700 rounded-full transition-all duration-150
          ${
            collapsed
              ? ""
              : "ring-2 ring-inset ring-offset-4 ring-black-900 dark:ring-slate-400 bg-slate-900 dark:bg-slate-400 dark:ring-offset-slate-700"
          }
          `}
        ></div>
      )}
    </div>
  );
};

export default SidebarLogo;